...
export function parseThreadListFilter(queryObj: {
  ...
  return { page, pageSize, search, sort, categorySlug }
}
...
export async function createdThread(params: {
  ...
  const { categorySlug, authorUserId, body, title } = params
...
export async function listThreads(filter: ThreadListFilter): Promise<ThreadSummary[]> {
  ...
  return result.rows.map(mapthreadDetailSummaryRow)
}
